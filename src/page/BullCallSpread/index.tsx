import BullCallSpreadFilter from "./BullCallSpreadFilter"
...
import BullCallSpreadChartModal from "./BullCallSpreadChartModal"



type Props = {}

const BullCallSpread = (props: Props) => {


    const paramsFromLocalStorage = useSelector(
        (state: any) => state.BullCallSpreadFilterSlice // Replace with the actual slice name used in the store
    );
    ...
    const getArbitrage = async () => {
        const response = await axios.get(`spreads_strategies/bull_call`, {
            params: paramsFromLocalStorage,
        });
        return response.data;
    };

    const { data, isFetching, isError, refetch }: { data: any, isFetching: boolean, isError: boolean, refetch: any } = useQuery('bull_call', getArbitrage)
    ...
    const columns = data && generateColumnsArray(data?.metadata?.column_names, [
        {
            header: 'نماد',
            accessorKey: 'short_ticker',
            Cell: (row: any) => {
                const val = row.value
                return renderTicker(val, row.row.original.short_tse_code)
            },
        },
        {
            header: 'نماد',
            accessorKey: 'long_ticker',
            Cell: (row: any) => {
                const val = row.value
                return renderTicker(val, row.row.original.long_tse_code)
            },
        },
        {
            header: 'تحلیل',
            ...
            Cell: (row: any) => {
                return <div className='w-full justify-center flex items-center cursor-pointer text-center'>
                    <Dialog>
                        ...
                        <DialogContent className='max-w-7xl mx-2 h-3/4'>
                            <BullCallSpreadChartModal row={row.row.original} />
                        </DialogContent>
                    </Dialog>
                </div>
            },
    ...
    return (
        <div className="bullCallTable">
            <BullCallSpreadFilter />
